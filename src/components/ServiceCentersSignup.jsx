...
function ServiceCenterSignUp() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (emailError || !email || !password || !name || !certification || !address) {
    ...
    try {
      ...
      navigate('/service-center-home'); // Redirect to dummy service center home
    } catch (err) {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      ...
      <form onSubmit={handleRegister} className="flex flex-col gap-5 w-full">
        ...
        <input type="text" placeholder="Certification Number" value={certification} onChange={(e) => setCertification(e.target.value)} required disabled={loading} className="p-4 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-500 text-gray-800 bg-gray-50 placeholder-gray-400 transition-all duration-300 ease-in-out" />
        <input type="text" placeholder="Address" value={address} onChange={(e) => setAddress(e.target.value)} required disabled={loading} className="p-4 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-500 text-gray-800 bg-gray-50 placeholder-gray-400 transition-all duration-300 ease-in-out" />
