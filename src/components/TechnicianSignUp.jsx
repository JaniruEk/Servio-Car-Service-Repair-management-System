...
function TechnicianSignUp() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (emailError || !email || !password || !name || !specialization || !age) {
    ...
    try {
      ...
      navigate('/technician-home'); // Redirect to dummy technician home
    } catch (err) {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      ...
      <form onSubmit={handleRegister} className="flex flex-col gap-5 w-full">
        ...
        <input type="text" placeholder="Specialization (e.g., Electrical)" value={specialization} onChange={(e) => setSpecialization(e.target.value)} required disabled={loading} className="p-4 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-500 text-gray-800 bg-gray-50 placeholder-gray-400 transition-all duration-300 ease-in-out" />
        <input type="number" placeholder="Age" value={age} onChange={(e) => setAge(e.target.value)} required min="18" disabled={loading} className="p-4 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-500 text-gray-800 bg-gray-50 placeholder-gray-400 transition-all duration-300 ease-in-out" />
